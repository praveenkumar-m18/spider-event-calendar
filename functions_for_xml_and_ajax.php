...
function spiderbigcalendar() {
	...
	//echo $a = "SELECT * FROM ". $wpdb->prefix . "spidercalendar_custom_content_piece WHERE id=".$eventID;
	$content_piece = $wpdb->get_results("SELECT * FROM ". $wpdb->prefix . "spidercalendar_custom_content_piece WHERE event_id=".$eventID);
	$content_rowcount = $wpdb->num_rows;
				...
				<tbody>
					<tr>
						<td>
							...
							echo '<b>Campaign Name:</b> <div style="color:' . $title_color . ';font-size:' . $title_size . 'px; font-family:' . $title_font . '; ' . $font_weight . '; ' . $font_style . '  "><a href="manage-campaign?action=view&&id='.$eventID .'" target="__blank" >' . $row_title . '</a></div>';
							
							
							/*if ($row->text_for_date != '') {
								echo '<b>Content Piece</b><div style="line-height:20px">' . $pop_content . '</div>';
							}
							else {
